...
public class MergeSorter<T extends Comparable<T>> {
	...
	public Queue<T> mergeSort(Queue<T> queue) {
		...
		Queue<T> output2 = new Queue<T>();	// output queue 2
        	// TODO 1
			//Basically just implemented the above method
	...
	void split(Queue<T> input, Queue<T> output1, Queue<T> output2) {
        	// TODO 2
		//Essentially checking if the input Queue is empty then returning the method 
		if (input.isEmpty()) {
			return;
		}
		//enqueues the dequeue data from input and then checks if the list is empty again
		output1.enqueue(input.dequeue());
		
		if (input.isEmpty()) {
			return;
		}
		//if its not empty, it will enqueue the dequeue and then return the method
